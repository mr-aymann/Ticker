...
const getTheatresByShows=async(req, res, next) => {
  const { id } = req.params;
  const { city, state, date } = req.query; // Get location and date parameters

  console.log({ id, city, state, date });
   // Prepare values for LIKE search with wildcards
   const citySearch = city ? `%${city}%` : null;  // Wildcards for city
   const stateSearch = state ? `%${state}%` : null; // Wildcards for state

    try {
        // Query to get theatre and show data based on id, location, and date
        const result = await pool.query(`
           ...
           WHERE 
            ...
            ORDER BY 
                t.id, s.show_time;
        `, [id, citySearch, stateSearch, date]);

        // Format the result to group shows by theatre
        ...
        result.rows.forEach(row => {
            const { theater_id, theater_name, city, state, address, show_id, show_time, date, available_capacity, total_capacity, screen_number, is_active ,slot_id} = row;
